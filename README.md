...
この回の目的はレイトレーシングというレンダリング法を行うときの要点を知ることと、それを利用したパストレーシングというレンダリング法の考え方を知ることです。そのために穴埋め形式の問題を解きながら各手法を実装します。
以下で目次の直後にレイトレーシングとパストレーシングの説明がそれぞれ書いてあります。

実装については、テキスト本編ではあまり深入りしないで説明してるので、細かい仕様について疑問がある人はslack上で質問するか一番下の[仕様](#仕様)の項目を参照してください。
...
レイトレーシングについて歴史的な説明をします。

三次元座標系上に定義された物を二次元のスクリーン(画面)に描画することを3DCGにおけるレンダリングといいます。
そのための主要な課題として、遠近感など**視界から見える形状を再現する**という問題と、複数の物体が重なって見える場合に**形状の前後関係を正しく表示する**という問題がありました。
これらを解決するため、大きく分けて２つの方法が使われてきました。

一つはラスタライゼーションという考え方で、まずメッシュなどベクタ表現された形状をスクリーン空間に投影します(左上)。
次に投影された形状と交差する画素を塗りつぶす(右下)ことで画像を作ることができます。

<img src="docs/f_raster.jpg" width="400">


もう一つはレイトレーシングという考え方です。これは画像が持つそれぞれの画素において、直進する光線(レイ)を追跡することによってその先に何が見えるのかを計算し、色を決定します。

<img src="docs/f_rt.jpg" width="400">


ラスタライゼーションが "形状 → 交差する画素の塗りつぶし" という順で処理が進むのに対し、レイトレーシングは "各画素 → どの形状が見えるか" という順で処理をすすめるというのが大きな違いです。

ここでは配布するプログラムを各自で書き換えていき、各画素について物体との交点を求めるという部分を実装します。
