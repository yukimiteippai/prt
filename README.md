...
返り値として、交点があればその点の情報を返し、無ければ`null`を返します。
...
ふだん我々が変数を扱うとき、メモリ上のある領域にその実体が確保され、その場所を指すことで代入などの操作をおこなっています。しかし今回のように該当する交点が存在しない場合には、Hitクラスの実体を確保しないかわりに`null`というどの実体をも指さない位置を返すことでそのことを示します。
...
`trace`では本来あらゆる方向から入射してくる光のうち一方向だけを選んで追跡しています。これを経路のサンプリングといいます。
何本も経路をサンプルし平均することで広い入射方向をカバーし、より正解に近い結果を得ます。
