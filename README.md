...
三次元座標系上に定義された物を二次元のスクリーン(画面)に描画することを3DCGにおけるレンダリングといい、大きく分けて２つの考えに基づく方法があります。

一つはラスタライゼーションという考え方で、まずメッシュなどベクタ表現された形状をスクリーン空間に投影します。
次に投影された形状と交差する画素を塗りつぶすことで画像を作ることができます。

もう一つはレイトレーシングという考え方です。これは画像が持つそれぞれの画素において、直進する光線(レイ)を追跡することによってその先に何が見えるのかを計算し、色が決定します。
