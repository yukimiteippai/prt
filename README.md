...
いつも通りの処理なので問題ないと思います。今回は光の強さを直接計算しやすいよう、`colorMode(RGB, 1.0)`を使います。
...
### レイトレーシングの基本
レイトレーシングでは**視線の生成**, **交差判定**, 判定結果による**描画色の判断**という処理を各ピクセルに対して行うことを既に述べましたが、これを図にすると以下のようになります。

![](docs/f_raycast.png)

#### 視線の生成

まず、ピクセルの位置が決まることで**どこ**から**どの方向**を視線が向いているかが決定します。今回のプログラムでは、`Camera.ray`という関数を使って視線の方向を求めます。視点と視線の方向は`Ray`クラスに保持されます。
...
#### 交差判定
視線がわかったら、それが物体と交差するかを判定します。このために`Hit sphere.intersect(ray, tmin, tmax)`という関数を使います。引数はレイと探索したい物体との距離の区間です。区間にはとりあえず十分に広い範囲を設定します。
...
ふだん我々が変数を扱うときにはメモリ上のある領域にその実体が確保され、その場所を指すことで代入などの操作をおこなっています。しかし今回のように該当する交点が存在しない場合には、Hitクラスの実体を確保しないかわりに`null`というどこを指してもいない値を返すことでそのことを示します。

![](docs/f_null.png)


#### 描画色の決定
得られた`Hit` 情報を使って最終的な色を求めます。ここでは物体の交点があれば物体色を返し、無ければ背景色を返すという処理をしています。
...
2. 物体色を、法線の値を使って決めてください。法線は`Hit`クラスの中に格納されています。
![](docs/exb_normal.png)
